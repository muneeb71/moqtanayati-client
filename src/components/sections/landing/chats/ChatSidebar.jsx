                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="max-w-[200px] truncate text-[13.17px] leading-[17.5px] text-[#595C75]">
                      {user.messages && user.messages[0]?.senderId === user.userAId && (
                        <span className="font-medium">You: </span>
                      )}
                      <span className={!user.messages || user.messages[0]?.senderId !== user.userAId ? "font-medium text-moonstone" : ""}>
                        {user.messages && user.messages[0]?.content}
                      </span>
                    </p>
                  </div>
                </div>
                <span className="mt-2 text-[11px] leading-[17.5px] text-nowrap text-[#9799A8]">
                  {user.messages && user.messages[0]?.createdAt ? new Date(user.messages[0].createdAt).toLocaleString() : ""}
                </span>
              </div>
            ))
          )}
